...
// g++ aes128.cpp -o aes128 -lssl -lcrypto -O2
...
#define POS0(u) ((u) >> 24)
#define POS1(u) (((u) >> 16) & 0xff)
#define POS2(u) (((u) >> 8) & 0xff)
#define POS3(u) ((u)&0xff)

// T tables are copied from OpenSSL
...
#define EN_TRANS(s0, s1, s2, s3) Te0[POS0(s0)] ^ Te1[POS1(s1)] ^ Te2[POS2(s2)] ^ Te3[POS3(s3)]
#define S_BOX(t0, t1, t2, t3) (Te2[t0] & 0xff000000) ^ (Te3[t1] & 0x00ff0000) ^ (Te0[t2] & 0x0000ff00) ^ (Te1[t3] & 0x000000ff)
#define S_BOX_(t0) (Te1[t0] & 0x000000ff)
#define EN_TRANS_LAST(t0, t1, t2, t3) S_BOX(POS0(t0), POS1(t1), POS2(t2), POS3(t3))

#define DE_TRANS(s0, s1, s2, s3) Td0[POS0(s0)] ^ Td1[POS1(s1)] ^ Td2[POS2(s2)] ^ Td3[POS3(s3)]
#define S_BOX_INV(t0, t1, t2, t3) (rsbox[t0] << 24) ^ (rsbox[t1] << 16) ^ (rsbox[t2] << 8) ^ (rsbox[t3])
#define DE_TRANS_LAST(t0, t1, t2, t3) S_BOX_INV(POS0(t0), POS1(t1), POS2(t2), POS3(t3))
#define INV_MIX_COLUMN(t) Td0[S_BOX_(POS0(t))] ^ Td1[S_BOX_(POS1(t))] ^ Td2[S_BOX_(POS2(t))] ^ Td3[S_BOX_(POS3(t))];
u32 round_keys[44];

void InitRoundKey(const u8 *key)
{
    u32 *rk = round_keys;
    rk[0] = GETU32(key);
    rk[1] = GETU32(key + 4);
    ...
    for (int i = 0; i < 10; i++)
    {
        rk[4] = rk[0] ^ S_BOX(POS1(rk[3]), POS2(rk[3]), POS3(rk[3]), POS0(rk[3])) ^ rcon[i];
        rk[5] = rk[1] ^ rk[4];
        rk[6] = rk[2] ^ rk[5];
        rk[7] = rk[3] ^ rk[6];
        rk += 4;
...
void InitRoundKey_inv(const u8 *key)
{
    u32 *rk = round_keys + 40;
    int i;
    rk[0] = GETU32(key);
    rk[1] = GETU32(key + 4);
    rk[2] = GETU32(key + 8);
...
void aes128_encrypt_block(const u8 *in, u8 *out)
{
    const u32 *rk = round_keys;
    u32 s0, s1, s2, s3, t0, t1, t2, t3;
    s0 = GETU32(in) ^ rk[0];
    s1 = GETU32(in + 4) ^ rk[1];
    s2 = GETU32(in + 8) ^ rk[2];
    s3 = GETU32(in + 12) ^ rk[3];
    t0 = EN_TRANS(s0, s1, s2, s3) ^ rk[4];
    t1 = EN_TRANS(s1, s2, s3, s0) ^ rk[5];
    t2 = EN_TRANS(s2, s3, s0, s1) ^ rk[6];
    t3 = EN_TRANS(s3, s0, s1, s2) ^ rk[7];
    s0 = EN_TRANS(t0, t1, t2, t3) ^ rk[8];
    s1 = EN_TRANS(t1, t2, t3, t0) ^ rk[9];
    s2 = EN_TRANS(t2, t3, t0, t1) ^ rk[10];
    s3 = EN_TRANS(t3, t0, t1, t2) ^ rk[11];
    t0 = EN_TRANS(s0, s1, s2, s3) ^ rk[12];
    t1 = EN_TRANS(s1, s2, s3, s0) ^ rk[13];
    t2 = EN_TRANS(s2, s3, s0, s1) ^ rk[14];
    t3 = EN_TRANS(s3, s0, s1, s2) ^ rk[15];
    s0 = EN_TRANS(t0, t1, t2, t3) ^ rk[16];
    s1 = EN_TRANS(t1, t2, t3, t0) ^ rk[17];
    s2 = EN_TRANS(t2, t3, t0, t1) ^ rk[18];
    s3 = EN_TRANS(t3, t0, t1, t2) ^ rk[19];
    t0 = EN_TRANS(s0, s1, s2, s3) ^ rk[20];
    t1 = EN_TRANS(s1, s2, s3, s0) ^ rk[21];
    t2 = EN_TRANS(s2, s3, s0, s1) ^ rk[22];
    t3 = EN_TRANS(s3, s0, s1, s2) ^ rk[23];
    s0 = EN_TRANS(t0, t1, t2, t3) ^ rk[24];
    s1 = EN_TRANS(t1, t2, t3, t0) ^ rk[25];
    s2 = EN_TRANS(t2, t3, t0, t1) ^ rk[26];
    s3 = EN_TRANS(t3, t0, t1, t2) ^ rk[27];
    t0 = EN_TRANS(s0, s1, s2, s3) ^ rk[28];
    t1 = EN_TRANS(s1, s2, s3, s0) ^ rk[29];
    t2 = EN_TRANS(s2, s3, s0, s1) ^ rk[30];
    t3 = EN_TRANS(s3, s0, s1, s2) ^ rk[31];
    s0 = EN_TRANS(t0, t1, t2, t3) ^ rk[32];
    s1 = EN_TRANS(t1, t2, t3, t0) ^ rk[33];
    s2 = EN_TRANS(t2, t3, t0, t1) ^ rk[34];
    s3 = EN_TRANS(t3, t0, t1, t2) ^ rk[35];
    t0 = EN_TRANS(s0, s1, s2, s3) ^ rk[36];
    t1 = EN_TRANS(s1, s2, s3, s0) ^ rk[37];
    t2 = EN_TRANS(s2, s3, s0, s1) ^ rk[38];
    t3 = EN_TRANS(s3, s0, s1, s2) ^ rk[39];
    s0 = EN_TRANS_LAST(t0, t1, t2, t3) ^ rk[40];
    PUTU32(out, s0);
    s1 = EN_TRANS_LAST(t1, t2, t3, t0) ^ rk[41];
    PUTU32(out + 4, s1);
    s2 = EN_TRANS_LAST(t2, t3, t0, t1) ^ rk[42];
    PUTU32(out + 8, s2);
    s3 = EN_TRANS_LAST(t3, t0, t1, t2) ^ rk[43];
    PUTU32(out + 12, s3);
}
void aes128_decrypt_block(const u8 *in, u8 *out)
{
    const u32 *rk = round_keys;
    u32 s0, s1, s2, s3, t0, t1, t2, t3;
    s0 = GETU32(in) ^ rk[0];
    ...
    s2 = GETU32(in + 8) ^ rk[2];
    s3 = GETU32(in + 12) ^ rk[3];
    t0 = DE_TRANS(s0, s3, s2, s1) ^ rk[4];
    t1 = DE_TRANS(s1, s0, s3, s2) ^ rk[5];
    t2 = DE_TRANS(s2, s1, s0, s3) ^ rk[6];
    t3 = DE_TRANS(s3, s2, s1, s0) ^ rk[7];
    s0 = DE_TRANS(t0, t3, t2, t1) ^ rk[8];
    s1 = DE_TRANS(t1, t0, t3, t2) ^ rk[9];
    s2 = DE_TRANS(t2, t1, t0, t3) ^ rk[10];
    s3 = DE_TRANS(t3, t2, t1, t0) ^ rk[11];
    t0 = DE_TRANS(s0, s3, s2, s1) ^ rk[12];
    t1 = DE_TRANS(s1, s0, s3, s2) ^ rk[13];
    t2 = DE_TRANS(s2, s1, s0, s3) ^ rk[14];
    t3 = DE_TRANS(s3, s2, s1, s0) ^ rk[15];
    s0 = DE_TRANS(t0, t3, t2, t1) ^ rk[16];
    s1 = DE_TRANS(t1, t0, t3, t2) ^ rk[17];
    s2 = DE_TRANS(t2, t1, t0, t3) ^ rk[18];
    s3 = DE_TRANS(t3, t2, t1, t0) ^ rk[19];
    t0 = DE_TRANS(s0, s3, s2, s1) ^ rk[20];
    t1 = DE_TRANS(s1, s0, s3, s2) ^ rk[21];
    t2 = DE_TRANS(s2, s1, s0, s3) ^ rk[22];
    t3 = DE_TRANS(s3, s2, s1, s0) ^ rk[23];
    s0 = DE_TRANS(t0, t3, t2, t1) ^ rk[24];
    s1 = DE_TRANS(t1, t0, t3, t2) ^ rk[25];
    s2 = DE_TRANS(t2, t1, t0, t3) ^ rk[26];
    s3 = DE_TRANS(t3, t2, t1, t0) ^ rk[27];
    t0 = DE_TRANS(s0, s3, s2, s1) ^ rk[28];
    t1 = DE_TRANS(s1, s0, s3, s2) ^ rk[29];
    t2 = DE_TRANS(s2, s1, s0, s3) ^ rk[30];
    t3 = DE_TRANS(s3, s2, s1, s0) ^ rk[31];
    s0 = DE_TRANS(t0, t3, t2, t1) ^ rk[32];
    s1 = DE_TRANS(t1, t0, t3, t2) ^ rk[33];
    s2 = DE_TRANS(t2, t1, t0, t3) ^ rk[34];
    s3 = DE_TRANS(t3, t2, t1, t0) ^ rk[35];
    t0 = DE_TRANS(s0, s3, s2, s1) ^ rk[36];
    t1 = DE_TRANS(s1, s0, s3, s2) ^ rk[37];
    t2 = DE_TRANS(s2, s1, s0, s3) ^ rk[38];
    t3 = DE_TRANS(s3, s2, s1, s0) ^ rk[39];
    s0 = DE_TRANS_LAST(t0, t3, t2, t1) ^ rk[40];
    PUTU32(out, s0);
    s1 = DE_TRANS_LAST(t1, t0, t3, t2) ^ rk[41];
    PUTU32(out + 4, s1);
    s2 = DE_TRANS_LAST(t2, t1, t0, t3) ^ rk[42];
    PUTU32(out + 8, s2);
    s3 = DE_TRANS_LAST(t3, t2, t1, t0) ^ rk[43];
    PUTU32(out + 12, s3);
}

void aes128_cbc_encrypt(const u8 *in, u8 *out, int len, const u8 *key, const u8 *ivec)
{
    InitRoundKey(key);
    int n;
    const u8 *iv = ivec;
...
void aes128_cbc_decrypt(const u8 *in, u8 *out, int len, const u8 *key, const u8 *ivec)
{
    InitRoundKey_inv(key);
    int n;
    const u8 *iv = ivec;
    assert(len > 0 && len % 16 == 0);
    while (len)
...
void test()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        random_fill(key, 16);
        random_fill(iv, 16);
        aes128_cbc_encrypt(text, buffer, num_bytes, key, iv);
        aes128_cbc_decrypt(buffer, buffer2, num_bytes, key, iv);
        AES_set_encrypt_key(key, 128, &aes);
        AES_cbc_encrypt(text, target, num_bytes, &aes, iv, AES_ENCRYPT);
        assert_eq(buffer, target, num_bytes);
        assert_eq(text, buffer2, num_bytes);
    }
}
...
void bench()
{
    ...
    printf("encrypt: %lf Mbps\n", num_bytes * 8 * times / 1024 / 1024 / tm);
    t0 = clock();
    for (int i = 0; i < times; i++)
    {
        aes128_cbc_decrypt(text, buffer, num_bytes, key, iv);
    }
    tm = (clock() - t0) / CLOCKS_PER_SEC;
    printf("decrypt: %lf Mbps\n", num_bytes * 8 * times / 1024 / 1024 / tm);
}
